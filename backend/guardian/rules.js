...
const RULES = [
  ...
  { id: "no-internal-leak", rule: "Internal-only data (internal_notes, potential_risks, confidence, complexity, priority, missing_information, and the AI's own internalAnalysisMarkdown synthesis) must never leak into client-facing output — the outreach email body/subject or the accompanying text message. ai/emailPrompt.js's draftEmail operation is given these fields as input for its own strategic reasoning (see its STEP 1-10), but ai/emailSchema.js keeps internalAnalysisMarkdown as a separate field from subject/body/textMessage specifically so the app never sends or displays it to a client, and the prompt itself explicitly forbids referencing internal-only facts or the analysis section's existence in the client-facing fields." },
  ...
  { id: "pricing-strategy-advisory-only", rule: "ai/aiService.js's generatePricingStrategy() output (the Pricing & Offer Strategy) is always a preliminary internal estimate, never a quote or commitment — it must never be written to contracts.price or any other authoritative record automatically, and no code path may auto-populate a contract from it. The admin remains the only one who ever sets a real contract price, by hand, in the existing Contracts feature. The prompt must never fabricate a client budget, market data, or competitor pricing not present in the actual project context, and must never use the $1,500-per-feature anchor as a bare multiplication (anchor x feature count) presented as the final price." },
];
