...
- **Known remaining uncertainty**: this assumes the deploy container is granted a real TUN network device (needed for `tailscale up` to actually create a working network interface) — if Railway's containers don't grant that, `tailscale up` will fail with a permission-related error in the logs (caught by the safe-failure design — the app still boots fine either way) and the fix would be switching Tailscale to "userspace networking" mode plus routing the app's Ollama requests through a local proxy, which is a real code change to `ai/providers/ollamaProvider.js`, not yet built. Check the `[tailscale]` log lines after a deploy to see which case applies.
